/* Unit5_Assignment_Part1_Bui.cpp : This file contains the 'main' function. Program execution begins and ends there.
Contains function weeklyPay that returns weekly pay.
...
int main()
{
	string partTwoA = "TCC-TR IS COOL";
	string partTwoB = "i will succeed at c++";
	locale loc;
	string reset = "\x1b[0m";

	//a function call for each person - no input from the console for part 1
	cout << fixed << setprecision(2);
	cout << weeklyPay(47.0, 505.75, "Daniel", 1) << endl;
	...
	for (int i = 0; i < partTwoA.length(); i++)
	{
		cout << tolower(partTwoA[i], loc);
	}
	cout << reset << endl; //a line after the name
	
	for (int i = 0; i < partTwoB.length(); i++)
	{
		cout << toupper(partTwoB[i], loc);
	}
	cout << reset << endl; //a line after the name
